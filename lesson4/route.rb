...
class Route
  ...
  def validate!
    raise 'Нет начальной станции!' if Station.all_stations.reject { |station| station.name == @first_station }.empty?
    raise 'Нет конечной станции!' if Station.all_stations.reject { |station| station.name == @last_station }.empty?
  end
